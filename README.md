...
На экране входа переключатель **«Использовать моковый запрос»** задаёт режим:

- **Мок включён** — вход выполняется локально, без сетевого запроса
- **Мок выключен** — запрос уходит на публичный API **DummyJSON**: базовый адрес `https://dummyjson.com`

Чтобы вход **без мока** прошёл успешно, нужны **логин и пароль существующего пользователя** DummyJSON (в теле запроса логин передаётся как `username`). Примеры и список пользователей: [документация авторизации](https://dummyjson.com/docs/auth), [список пользователей](https://dummyjson.com/users)

Например, можно использовать пару: `avat` / `avatpass`
